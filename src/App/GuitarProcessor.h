...
public:
    ...
    void setRigMode(RigMode next);
    RigMode getRigMode() const { return (RigMode) rigMode.load(std::memory_order_relaxed); }
    ...
    void setTunerArmed(bool should) { tuner.setArmed(should); }
    bool isTunerArmed() const { return tuner.isArmed(); }
    TunerReading getTunerReading()
    {
        tuner.analyse();
...
private:
    ...
    TapeEngine tape;
    LooperEngine looper;
    VocalChain liveVocal;
    std::array<VocalChain, TapeEngine::numLanes> laneVocals;
    VocalChain exportVocal;
    std::array<std::atomic<float>, 5> vocalAxes {};
