...
public:
    ...
    void setBank(const juce::String& key)
    {
        bank = key.isNotEmpty() ? key : "presets";
        load();
    }

    const juce::String& getBank() const { return bank; }
    ...
    void load()
    {
        ...
        std::unique_ptr<juce::XmlElement> xml(appSettings().getXmlValue(bank));
        if (xml == nullptr)
            return;
    ...
    void save() const
    {
        ...
        appSettings().setValue(bank, xml.get());
