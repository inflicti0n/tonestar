                           ...
                           juce::String& error)
{
    ...
    mix.clear();
    auto* left = mix.getWritePointer(0);
    auto* right = mix.getWritePointer(1);
    for (int s = 0; s < frames; ++s)
    {
        float mixL = 0.0f;
        float mixR = 0.0f;
        const int head = start + s;
        for (int i = 0; i < numLanes; ++i)
    ...
    for (int s = 0; s < numSamples; ++s)
    {
        ...
        if (transport == Transport::Recording)
        {
                     ...
                     && lanes[recLane].audio.getNumSamples() > recFrames)
            {
                ...
                const float liveL = gL[s];
                const float liveR = gR[s];
        ...
        if (transport == Transport::Playing || transport == Transport::Recording)
        {
            ...
            for (int i = 0; i < numLanes; ++i)
            {
                if (transport == Transport::Recording && i == recLane)
                    continue;
                mixLane(lanes[i], playhead, mixL, mixR);
            }
            left[s] += mixL;
            right[s] += mixR;
            ++playhead;
