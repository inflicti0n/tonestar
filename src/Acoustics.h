...
private:
    ...
    void applyRecipe(float size, float back)
    {
        ...
        const float couplingDb = juce::jmap(size, 0.0f, 3.8f);
        const float darkHz = juce::jmap(size, 9800.0f, 6200.0f);
        const float beamDb = juce::jmap(size, 0.2f, -3.4f);
        const float closedLowDb = juce::jmap(back, -1.4f, 3.6f);
        const float leakDb = juce::jmap(back, 2.4f, -2.2f);
        const float floorHz = juce::jmap(size, 2400.0f, 1800.0f);
        const float tailHz = juce::jmap(back, 1700.0f, 1250.0f);
