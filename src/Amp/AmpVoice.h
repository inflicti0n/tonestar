...
public:
    ...
    void prepare(const juce::dsp::ProcessSpec& spec)
    {
        ...
        probeOs.initProcessing((size_t) probeLength);
    ...
    void process(juce::AudioBuffer<float>& buffer, const AmpParams& target)
    {
        ...
        clipOversampled(oversampling, block, smoothed, true);
...
private:
    ...
    float measureProbe(const AmpParams& p)
    {
        ...
        clipOversampled(probeOs, block, p, false);
    ...
    void clipOversampled(juce::dsp::Oversampling<float>& os,
                         juce::dsp::AudioBlock<float>& block,
                         const AmpParams& p, bool live)
    {
        auto up = os.processSamplesUp(juce::dsp::AudioBlock<const float>(block));
        clip(up.getChannelPointer(0), (int) up.getNumSamples(), p, live,
             sampleRate * (double) (1 << oversampleStages));
        os.processSamplesDown(block);
    }

    static float softStage(float x)
    {
        return x / (1.0f + std::abs(x));
    }

    void clip(float* data, int n, const AmpParams& p, bool live, double sr)
    {
        ...
        const float rateScale = (float) (sampleRate / juce::jmax(1.0, sr));
        const float sagCoeff = 0.0045f * rateScale;
        const float gateAtk = 0.35f * rateScale;
        const float gateRel = 0.04f * rateScale;
        ...
        for (int i = 0; i < n; ++i)
        {
            ...
            {
                gateEnv += (std::abs(x) - gateEnv) * (std::abs(x) > gateEnv ? gateAtk : gateRel);
    ...
    juce::dsp::Oversampling<float> oversampling { 1, oversampleStages,
        juce::dsp::Oversampling<float>::filterHalfBandPolyphaseIIR };
    juce::dsp::Oversampling<float> probeOs { 1, oversampleStages,
        juce::dsp::Oversampling<float>::filterHalfBandPolyphaseIIR };
