...
float sdPoly(juce::Point<float> p, const std::array<juce::Point<float>, 6>& verts, int n)
{
    n = juce::jlimit(3, 6, n);
                         ...
                         int nextCount)
{
    ...
    nextCount = juce::jlimit(3, 6, nextCount);
    const bool changed = nextW != logicalW || nextH != logicalH
                         || nextCount != vertCount
                         || vertsMoved(verts, nextVerts, nextCount);
    logicalW = nextW;
    logicalH = nextH;
    centre = nextCentre;
    verts = nextVerts;
...
{
    const float centreSd = sdPoly(centre, verts, vertCount);
    ...
    for (int j = 0; j < N; ++j)
    {
        for (int i = 0; i < N; ++i)
        {
            ...
            const float sd = sdPoly({ px, py }, verts, vertCount);
