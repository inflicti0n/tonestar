...
AdvancedDrawer::AdvancedDrawer()
{
    ...
    list.onSelectClip = [this](int lane)
    {
...
void AdvancedDrawer::resized()
{
    ...
    auto footer = bounds.removeFromBottom(80);
    const int splitGap = 20;
    auto metro = footer.removeFromLeft(juce::jmax(80, (footer.getWidth() - splitGap) / 2));
    footer.removeFromLeft(splitGap);
    auto tune = footer;
    const int ctrl = 28;
    const int bpmW = 52;
    const int rowY = footer.getY() + (footer.getHeight() - ctrl) / 2;
    metroButton.setBounds(metro.getX(), rowY, ctrl, ctrl);
    bpmField.setBounds(metro.getRight() - bpmW, rowY, bpmW, ctrl);
    sparkle.setBounds(metro.withSizeKeepingCentre(80, 80));
    tunerButton.setBounds(tune.getX(), rowY, ctrl, ctrl);
    tunerFace.setBounds(tune.withTrimmedLeft(ctrl + 10));
